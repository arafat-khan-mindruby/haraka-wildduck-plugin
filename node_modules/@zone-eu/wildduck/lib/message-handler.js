...
class MessageHandler {
    ...
    async addAsync(options) {
        ...
        // Option A Fix 1: Unset previous latestInThread, then insert new message as latest
        messageData.latestInThread = true;
        try {
            await this.database.collection('messages').updateMany(
                { mailbox: messageData.mailbox, thread: messageData.thread, latestInThread: true },
                { $set: { latestInThread: false } },
                { writeConcern: 'majority' }
            );
        } catch (updateErr) {
            log.error('latestInThread', 'Failed to unset latestInThread in addAsync(): %s', updateErr.message);
        }
    ...
    async delAsync(options) {
        ...
        // Option A Fix 1: If deleted message was latest in thread, promote next most recent
        if (messageData.latestInThread) {
            try {
                await this.database.collection('messages').findOneAndUpdate(
                    { mailbox: messageData.mailbox, thread: messageData.thread },
                    { $set: { latestInThread: true } },
                    { sort: { idate: -1 }, writeConcern: 'majority' }
                );
            } catch (promoteErr) {
                log.error('latestInThread', 'Failed to promote latestInThread in delAsync(): %s', promoteErr.message);
            }
        }
    ...
    async moveAsync(options) {
        ...
        let encryptedMoveUserData;
        if (targetData.encryptMessages) {
            try {
                encryptedMoveUserData = await this.users.collection('users').findOne(
                    { _id: mailboxData.user },
                    {
                        projection: {
                            pubKey: true
        ...
        while (message !== undefined) {
            ...
            if (targetData.encryptMessages) {
                ...
                if (parsedContentType && parsedContentType.subtype === 'encrypted') {
                    ...
                    // get user data
                    if (!encryptedMoveUserData?.pubKey) {
                    ...
                    await new Promise((resolve, reject) => {
                        outputStream
                            ...
                            .on('end', async () => {
                                ...
                                try {
                                    encryptRes = await this.encryptMessageAsync(encryptedMoveUserData.pubKey, raw);
                                } catch (err) {
                                    return reject(
        ...
        getMailbox((err, mailboxData) => {
            ...
            this.database.collection('mailboxes').findOneAndUpdate(
                ...
                (err, item) => {
                    ...
                    // Option A Fix 1: Mark as latest in thread, unset previous
                    messageData.latestInThread = true;
                    this.database.collection('messages').updateMany(
                        ...
                        (updateErr) => {
                            if (updateErr) {
                                log.error('latestInThread', 'Failed to unset latestInThread in put(): %s', updateErr.message);
                            }
                    ...
                    }); // end updateMany callback (Option A Fix 1 - put)
                }
            );
        });
    }
    ...
    async updateMessage(data, cursor, options) {
        ...
        // Option A Fix 1: Mark as latest in target mailbox thread
        message.latestInThread = true;
        try {
            await this.database.collection('messages').updateMany(
                { mailbox: message.mailbox, thread: message.thread, latestInThread: true },
                { $set: { latestInThread: false } },
                { writeConcern: 'majority' }
            );
        } catch (updateErr) {
            log.error('latestInThread', 'Failed to unset latestInThread in move(): %s', updateErr.message);
        }
        ...
        if (deleteMessageRes && deleteMessageRes.deletedCount) {
            ...
            // Option A Fix 1: Recalculate latestInThread in source mailbox after move-delete
            try {
                await this.database.collection('messages').findOneAndUpdate(
                    { mailbox: mailboxData._id, thread: message.thread },
                    { $set: { latestInThread: true } },
                    { sort: { idate: -1 }, writeConcern: 'majority' }
                );
            } catch (promoteErr) {
                log.error('latestInThread', 'Failed to promote latestInThread in move-delete(): %s', promoteErr.message);
            }
        }
